...
* **🧠 Autonomous Headless ABAP Architect (MCP):** A state-of-the-art developer agent that bypasses the physical GUI. Utilizing the **Model Context Protocol (MCP)** and SAP's **ABAP Development Tools (ADT)** bridge, the bot can headlessly create, modify, and activate ABAP structures, classes, and programs directly in the backend via Google Gemini.  
...
* **🔌 Enterprise SAP Integration (Hybrid Architecture):** * **Headless Developer Mode (MCP + ADT):** Uses Google Gemini via a Single-Pass MCP bridge to perform pure-code ABAP injections. It generates an execution plan, creates SE11/SE24/SE38 objects, and activates them without requiring a Windows host.  
  * **RFC Mode:** Securely connects to SAP backends via TCP/RFC (`node-rfc` + SAP C++ SDK) to pull headless data straight from the database. (e.g., ST22 Shortdumps, SLG1 Application Logs).  
